# AGENTS.md - Agent Guide for IMDb Crawler
...
**This repository's role:** Acquisition stage. Downloads and bulk-imports the official IMDb TSV datasets into `T_WC_IMDB_*` staging tables, enabling TMDb↔IMDb cross-referencing (IDs, ratings, alternate titles, principals, crew, episodes). The ratings and links are consumed downstream by `tmdb-movie-preprocess` when it builds the `T_WC_T2S_*` read-model.
...
The crawler is built and run as a Docker container via the repo's `Dockerfile` (base image `python:3.10.5-slim-buster`). It installs `requirements.txt`, copies the repo, and runs `CMD ["python", "./imdb-crawler.py"]`. No ports or volumes are declared; the MariaDB target and other settings come from runtime configuration.
...
- This repo: `C:\Users\vaugo\Nestor\projets\t2s-backlog\repos\imdb-crawler.md`
- Cross-repo dashboard: `C:\Users\vaugo\Nestor\projets\t2s-backlog\index.md`

Consult it before implementing: tasks are `IMDB-CRAWLER-NNN` with status (done / in-progress /
